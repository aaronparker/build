...
BeforeDiscovery {
    ...
    if ([System.String]::IsNullOrWhiteSpace($Env:GITHUB_WORKSPACE)) {
        $Path = [System.IO.Path]::Combine($PWD.Path, "rds")
    }
    else {
        $Path = [System.IO.Path]::Combine($Env:GITHUB_WORKSPACE, "rds")
    }
    ...
    $3rdPartyScripts = Get-ChildItem -Path $Path -Include "4*.ps1" -Recurse -Exclude "998_Bisf.ps1"
